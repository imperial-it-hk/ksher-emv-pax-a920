...
class KeyboardPasswordUtil : View.OnClickListener {
    private var amountLong = ""
    private val decimal = 2

    //    private boolean keypadToneEnabled = SystemParam.keypadToneEnabled.get();
    private var listener: KeyboardPasswordListener? = null
    private var keypadEnabled = true
    fun init(keyContainer: ViewGroup, listener: KeyboardPasswordListener?) {
        // Bind listener
        var i = 0
...
//        if (amtStr.toLong() == 0L) return
        ...
        // The maximum length does not exceed 15
        if (amtStr.length >= 15) {
            amtStr = amtStr.substring(0, 15)
        }
        amountLong = amtStr
        updateAmount()
    }
    ...
    private fun updateAmount() {
        if (listener != null) {
            listener?.updateAmount(amountLong)
        }
    }
    ...
    override fun onClick(v: View) {
        ...
        when (v.id) {
            ...
            R.id.key_c -> clear()
            R.id.key_del -> delete()
            R.id.key_confirm -> if (listener != null) {
                listener?.confirm()
            }
            R.id.key_cancel -> if (listener != null) {
                listener?.cancel()
            }
        }
    }

    interface KeyboardPasswordListener {
