 ...
 */
public class QRShareGenerator extends BaseReceiptGenerator {
    ...
    protected QRShareGenerator(TransDataModel transData) {
        super();
        this.transData = transData;
    }

    public QRShareGenerator(TransDataModel transData, int slipNum, boolean isReprint) {
        super();
...
//        parsePrintingMarkups(transData.getSlipPrintingMarkups());
        if (printingMarkupsList.size() > 0) {
            ...
            if (!TextUtils.isEmpty(transData.getExchangeRate()) && new BigDecimal(transData.getExchangeRate()).compareTo(BigDecimal.ONE) != 0) {
...
//        }
        ...
        feedLine();
        addCopyType();
        page.addLine().addUnit("DOLFIN V" + BuildConfig.VERSION_NAME, PrintFontSize.FONT_SMALL, CENTER, TEXT_STYLE_BOLD);
        if (slipNum == 1) {
            feedLine();
            addPromotionLogo();
        }
