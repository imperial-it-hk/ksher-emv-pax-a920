...
public class ProgressWheel extends View {
    ...
    public ProgressWheel(Context context, AttributeSet attrs) {
        ...
        parseAttributes(context.obtainStyledAttributes(attrs, com.evp.payment.ksher.R.styleable.ProgressWheel));
    }
    ...
    private void setupBounds(int layoutWidth, int layoutHeight) {
        ...
        if (!fillRadius) {
            ...
            float xOffset = (layoutWidth - paddingLeft - paddingRight - circleDiameter) / 2f + paddingLeft;
            float yOffset = (layoutHeight - paddingTop - paddingBottom - circleDiameter) / 2f + paddingTop;

            circleBounds = new RectF(xOffset + barWidth, yOffset + barWidth, xOffset + circleDiameter - barWidth,
                    yOffset + circleDiameter - barWidth);
        } else {
            circleBounds = new RectF((float) (paddingLeft + barWidth), (float) (paddingTop + barWidth), (float) (layoutWidth - paddingRight
                    - barWidth), (float) (layoutHeight - paddingBottom - barWidth));
        }
    }
    ...
    private void parseAttributes(TypedArray a) {
        ...
        circleRadius = (int) a.getDimension(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_circleRadius, circleRadius);

        fillRadius = a.getBoolean(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_fillRadius, false);

        barWidth = (int) a.getDimension(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_barWidth, barWidth);

        rimWidth = (int) a.getDimension(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_rimWidth, rimWidth);

        float baseSpinSpeed = a.getFloat(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_spinSpeed, spinSpeed / 360.0f);
        spinSpeed = baseSpinSpeed * 360;

        barSpinCycleTime = a.getInt(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_barSpinCycleTime, (int) barSpinCycleTime);

        barColor = a.getColor(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_barColor, barColor);

        rimColor = a.getColor(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_rimColor, rimColor);

        linearProgress = a.getBoolean(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_linearProgress, false);

        if (a.getBoolean(com.evp.payment.ksher.R.styleable.ProgressWheel_matProg_progressIndeterminate, false)) {
            spin();
        }
