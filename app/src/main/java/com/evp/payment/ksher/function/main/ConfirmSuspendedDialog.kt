...
open class ConfirmSuspendedDialog(context: Context, private val builder: Builder) :
    ...
    fun initViews() {
        ...
        if (builder.suspendedQrDataModel.paymentChannel!!.isNotEmpty()) {
            tvTitleImage.visible()
            setupSubHeaderLogo(tvTitleImage, builder.suspendedQrDataModel.paymentChannel.orEmpty())
        }
        ...
        bindingData(builder.suspendedQrDataModel)
    }
    ...
    private fun bindingData(transData: SuspendedQrDataModel) {
        tvTranTypeAmount.text = "SALE"
    ...
    fun confirmTransaction() {
        stopCountDown()
        builder.listener?.onSuccess(builder.suspendedQrDataModel, this) ?: dismiss()
    }

    @OnClick(R.id.btn_cancel)
    fun cancelTransaction() {
        stopCountDown()
        builder.listener?.onFail(builder.suspendedQrDataModel, this) ?: dismiss()
    }
    ...
    interface OnActionListener {
        fun onSuccess(transData: SuspendedQrDataModel, dialogFragment: ConfirmSuspendedDialog)
        fun onFail(transData: SuspendedQrDataModel, dialogFragment: ConfirmSuspendedDialog)
    }

    data class Builder(
        val context: Context, val title: String, val subtitle: String,
        val suspendedQrDataModel: SuspendedQrDataModel, var listener: OnActionListener? = null
    ) {
        fun build() = ConfirmSuspendedDialog(context, this)
        fun show() {
            build().show()
        }
    }
...
//                ivTitleImage.setImageResource(StringUtils.getImage(configModel.data?.menusMain?.get(0)?.payment?.get(0)?.icon))
//            }
